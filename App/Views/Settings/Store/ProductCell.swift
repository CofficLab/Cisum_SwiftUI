...
struct ProductCell: View {
    ...
    var buyButton: some View {
        ...
        .onAppear {
            Task {
                ...
                os_log("\(Logger.isMain) 🖥️ 💰 ListCell OnAppear 检查购买状态 -> \(product.displayName) -> \(isPurchased)")
            }
        }
    }
    ...
    func buy() async {
        ...
        do {
            os_log("\(Logger.isMain) 💰 点击了购买按钮")

            let result = try await store.purchase(product)
            if result != nil {
                withAnimation {
                    os_log("\(Logger.isMain) 🖥️ 💰 购买回调，更新购买状态为 true")
                    isPurchased = true
                }
            } else {
                os_log("\(Logger.isMain) 🖥️ 💰 购买回调，结果为空，表示取消了")
            }
