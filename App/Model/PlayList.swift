...
class PlayList {
    ...
    private func randomExcludeCurrent() -> AudioModel {
        if list.count == 1 {
            os_log("\(Logger.isMain)只有一条，随机选一条就是第一条")
            return AudioModel(list.first!)
        }

        let result = AudioModel((list.filter { $0 != audio.id }).randomElement()!)
        os_log("\(Logger.isMain)共 \(self.list.count) 条，随机选一条: \(result.title)")
...
extension PlayList {
    ...
    func saveToCache(_ url: URL) {
        os_log("\(Logger.isMain)DBModel::saveToCache")
    ...
    func isCached(_ url: URL) -> Bool {
        ...
        os_log("\(Logger.isMain)DBModel::isCached -> \(cachePath.absoluteString)")
        return fileManager.fileExists(atPath: cachePath.path)
    }

    func deleteCache(_ url: URL) {
        os_log("\(Logger.isMain)DBModel::deleteCache")
        if isCached(url), let cachedPath = getCachePath(url) {
            os_log("\(Logger.isMain)DBModel::deleteCache -> delete")
