...
/// 有声书仓库目录树的轻量加载容器（设置页「目录树」模式专用）。
///
/// 与主窗口内容区相互独立：设置页 onAppear 触发的重载不会传播到主窗口内容区。
/// 仅负责加载仓库目录树，不处理播放/选中态。
@MainActor
final class BookTreeViewModel: ObservableObject, SuperLog {
    nonisolated static let verbose = false

    @Published private(set) var children: [BookTreeNode] = []
    ...
    private func load() {
        ...
        Task { @MainActor in
            ...
            let children = await Task.detached(priority: .utility) {
                BookTreeBuilder.buildRootChildren(from: disk)
            }.value
